...
export const activeSkillProgress = () => {
  const progress_inner = document.querySelectorAll(".skillsInner___"),
    triggerBottom = (window.innerHeight / 10) * 10;
  progress_inner.forEach((box) => {
    ...
    if (boxTop < triggerBottom) {
      // Ensure boxItem is selected
      if (boxItem) {
        boxItem.classList.add("open");
        const barInElements = boxItem.getElementsByClassName("bar_in");
        // Ensure bar_in element exists
        if (barInElements.length > 0) {
          barInElements[0].style.width = `${pWidth}%`;
          barInElements[0].style.backgroundColor = pColor;
        } else {
          console.error("bar_in element not found in boxItem.");
        }
      } else {
        console.error("boxItem not found.");
      }
    }
  });
};
